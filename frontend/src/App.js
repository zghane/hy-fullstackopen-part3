	...
	const addPerson = (event) => {
		...
		if (!persons.map(person => person.name).includes(newName)) {
			...
			personService.create(newPerson).then(returnedPerson => {
				...
				displayNotification(`Error adding the person: ${error.response.data.error}`, true)
	...
	const deletePerson = (id) => {
		if (window.confirm(`Really delete the person?`)) {
			...
			personService.remove(id).then(response => {
				...
				displayNotification(`Failed to delete person (person has already been deleted?)\n${error.response.data.error}`, true)
			})
	...
	const updatePerson = (id, updatedNumber) => {
		...
		if (window.confirm(`${person.name} is already in the phonebook, replace the old number with a new one?`)) {
			personService.update(id, changedPerson).then(returnedPerson => {
				...
				displayNotification(`Failed to update ${person.name}'s information (already deleted?)\n${error.response.data.error}`, true)
			})
		}
	}
